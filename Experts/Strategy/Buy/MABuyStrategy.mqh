...
// #include <_MyInclude\IEntryRule.mqh>  // インターフェースは使用しない

//+------------------------------------------------------------------+
//| 移動平均線を使用した買いエントリー戦略                          |
//+------------------------------------------------------------------+
class MABuyStrategy // インターフェース継承を削除
{
private:
...
public:
    // コンストラクタ
    MABuyStrategy(int maPeriod = 12, int maShift = 6)
    {
        m_maPeriod = maPeriod;
        m_maShift = maShift;
    ...
    bool ShouldEnter(string symbol, ENUM_TIMEFRAMES timeframe) // override削除
    {
        ...
        return Signal(symbol, timeframe) == ORDER_TYPE_BUY;
    }
    ...
    double CalculateLot(string symbol, double accountBalance) // override削除
    {
        ...
        // 1ロットの必要証拠金
        double margin = 0;
        if(!OrderCalcMargin(ORDER_TYPE_BUY, symbol, 1.0, price, margin) || margin <= 0.0)
            return 0.01; // デフォルト値
